import os, json, argparse, socket, sqlite3, shutil, uuid
...
def get_settings():
    conn = db()
    row = conn.execute("SELECT base_url, api_key, model FROM settings WHERE id=1").fetchone()
    conn.close()
    return dict(row) if row else None

def upsert_settings(base_url: str, api_key: str, model: str):
    conn = db()
    conn.execute("INSERT INTO settings(id, base_url, api_key, model) VALUES(1,?,?,?) "
...
def extract_blocks(docx_path: str):
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        if text:
            blocks.append({"locator": f"p:{i}", "kind": "paragraph", "source_text": text, "order_no": order_no})
            order_no += 1
        ...
        for ri, row in enumerate(table.rows):
            for ci, cell in enumerate(row.cells):
                for pi, p in enumerate(cell.paragraphs):
                    text = (p.text or "").strip()
                    if text:
                        blocks.append({"locator": f"t:{ti}/r:{ri}/c:{ci}/p:{pi}",
...
async def create_task(file: UploadFile = File(...), direction: str = Form(...)):
    ...
    if not s or not s["base_url"] or not s["api_key"] or not s["model"]:
        raise HTTPException(400, "please set /api/settings first")
    ...
    # 澶嶅埗涓€浠戒綔涓?work_path锛圡VP 涔熷彲鐩存帴鐢ㄥ悓涓€浠斤級
    ...
    conn = db()
    conn.execute("INSERT INTO tasks(id, filename, source_path, work_path, direction, status, progress, error) VALUES(?,?,?,?,?,?,?,?)",
                 (task_id, file.filename, src_path, work_path, direction, "created", 0.0, None))
    for b in blocks:
        conn.execute("INSERT INTO blocks(id, task_id, locator, kind, order_no, source_text, translated_text, status) VALUES(?,?,?,?,?,?,?,?)",
                     (f"blk_{uuid.uuid4().hex}", task_id, b["locator"], b["kind"], b["order_no"], b["source_text"], None, "pending"))
    conn.commit()
    conn.close()
...
def get_task(task_id: str):
    conn = db()
    row = conn.execute("SELECT id, status, progress, error, direction FROM tasks WHERE id=?", (task_id,)).fetchone()
    conn.close()
    if not row:
...
def _translate_task(task_id: str):
    ...
    try:
        ...
        blocks = conn.execute("SELECT * FROM blocks WHERE task_id=? ORDER BY order_no ASC", (task_id,)).fetchall()
        total = len(blocks)
        done = 0
        ...
        for b in blocks:
            if b["status"] == "edited":
                ...
                conn.execute("UPDATE blocks SET translated_text=?, status=? WHERE id=?", (out, "translated", b["id"]))
                done += 1
        ...
        conn.execute("UPDATE tasks SET status=?, progress=? WHERE id=?", ("finished", 1.0, task_id))
        conn.commit()
...
def list_blocks(task_id: str, offset: int = 0, limit: int = 2000):
    conn = db()
    rows = conn.execute(
        "SELECT id, order_no, status, locator, kind, source_text, translated_text FROM blocks WHERE task_id=? ORDER BY order_no ASC LIMIT ? OFFSET ?",
...
def patch_block(task_id: str, block_id: str, payload: dict):
    ...
    conn = db()
    row = conn.execute("SELECT id FROM blocks WHERE id=? AND task_id=?", (block_id, task_id)).fetchone()
    if not row:
        conn.close()
        raise HTTPException(404, "block not found")
    conn.execute("UPDATE blocks SET translated_text=?, status=? WHERE id=?", (translated_text, "edited", block_id))
    conn.commit()
    conn.close()
    return {"ok": True}
...
def export_docx(task_id: str):
    conn = db()
    task = conn.execute("SELECT work_path, filename FROM tasks WHERE id=?", (task_id,)).fetchone()
    if not task:
        conn.close()
        raise HTTPException(404, "task not found")
    blocks = conn.execute("SELECT locator, translated_text FROM blocks WHERE task_id=?", (task_id,)).fetchall()
    conn.close()
...
def main():
    ...
    import uvicorn
    config = uvicorn.Config(app, host=args.host, port=port, fd=sock.fileno(), log_level="info")
